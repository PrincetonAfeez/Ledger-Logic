...
def detect_columns(headers: list[str], column_map: dict[str, Any] | None = None) -> ColumnMap:
    ...
    When ``column_map`` is omitted, uses the same header scoring as
    :func:`ledgerlogic.analysis.csv_columns.detect_columns` (date / merchant / amount only).
    When provided, starts from that auto map and overrides only keys present in ``column_map``.
    """
    from .analysis.csv_columns import detect_columns as _analysis_detect_columns

    full = _analysis_detect_columns(headers)
    resolved: ColumnMap = {
        "date": full["date"],
    ...
    for source_index, source_record in enumerate(source_records):
        ...
        for reference_index, reference_record in enumerate(reference_records):
            ...
            if not source_record["merchant_key"] or not reference_record["merchant_key"]:
                ...
                name_similarity = similarity_ratio(source_record["merchant_key"], reference_record["merchant_key"])
...
def build_report_text(
    ...
    lines = []
    lines.append("LedgerLogic Reconciliation Report")
    lines.append("=" * 40)
    lines.append(
...
def menu() -> None:
    ...
    while True:
        print()
        print("LedgerLogic: Expense Discrepancy Detector")
